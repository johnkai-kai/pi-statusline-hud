...
test("formatCount 進位後要重新判檔,不能吐出六欄", () => {
  ...
  for (const n of [999, 1000, 99_949, 99_950, 999_499, 999_500, 1_000_000]) {
    assert.ok(formatCount(n).length <= 5, `${n} -> ${formatCount(n)} 超過五欄`);
  }
});
