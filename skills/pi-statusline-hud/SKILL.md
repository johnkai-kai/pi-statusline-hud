...
5. `env` — AGENTS.md / MCPs / extensions / skills counts. Extensions and skills cover both the user level `<agentDir>/settings.json` and the project level `<cwd>/.pi/settings.json`. **MCP is not pi's** — pi's own resource types are extensions, skills, prompts and themes, and its source has no MCP discovery at all. The mechanism belongs to the third-party `pi-mcp-adapter` package: six shared and adapter-specific config files, each file's own compatibility `imports` (unaffected by `hostConfigDiscovery`), the automatic discovery of every host kind when `settings.hostConfigDiscovery` is `on`, and package `pi.mcp` entries — deduped by server name. Without that package installed the count still reads those files, so it describes configuration nobody loads. Two further caveats: extensions and skills are counted as *discovered*, not as *enabled* — a resource pi has switched off still counts — and the count is what the filesystem says, not what pi resolved.
